...
class gamevar:
	...
	var writable#: bool=true # Already set to true by default in the init.
...
onready var player_max_acc = gamevar.new(["player_max_acc"], "70", "Number", "The maximum acceleration the player can be applying")
onready var player_max_friction = gamevar.new(["player_max_friction"], "60", "Number", "The maximum friction that can be applied to the player")
onready var player_jump_speed = gamevar.new(["player_jump_speed"], "200", "Number", "The speed (or force) applied to the player when jumping")
